...
//~
//~ typedef enum e_eprFamilyPattern{	// quoi: Famille des patterns à chercher (strategie)
	//~ SAME_DICE_LOW,				// dés egaux (ie plusieurs 3) #1 #2 #3 poids bas
	//~ SAME_DICE_HIGH,				// dés egaux (ie plusieurs 3) #3 #4 #5 poids hauts
	//~ STRAIGHT_DICE,				// SS LS une suite (ie 2 3 4 or 4 5 6)
	//~ FULL_HOUSE,					// FH YAZ YB
	//~ FULL_HOUSE_WITH_MIN,		// ToK FoK et sum > 12
	//~ SUM_DICE,					// Sum
	//~ DIRECT_PATTERN,				// Figure entiere identifiée
	//~ OTHERS,						// Aucune des precedentes
//~ } eprFamilyPattern ;
//~
